...
static void volume_up_and_down_gpios_init(void)
{
	...
	if (retval < 0)
		printk(KERN_ERR"%s:GPIO%d request fault!%d\n", __func__, TEGRA_GPIO_PQ4, retval);

	retval = gpio_request(TEGRA_GPIO_PQ5, "VOL_UP_BUTTON");
	if (retval < 0)
		printk(KERN_ERR"%s:GPIO%d request fault!%d\n", __func__, TEGRA_GPIO_PQ5, retval);

	retval = gpio_direction_input(TEGRA_GPIO_PQ4);
	if (retval) {
		printk(KERN_ERR"%s:gpio_direction_input failed for input %d\n", __func__, TEGRA_GPIO_PQ4);
	}
	retval = gpio_direction_input(TEGRA_GPIO_PQ5);
	if (retval) {
		printk(KERN_ERR"%s:gpio_direction_input failed for input %d\n", __func__, TEGRA_GPIO_PQ5);
	}
}
